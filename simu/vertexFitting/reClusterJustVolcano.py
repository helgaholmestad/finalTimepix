...
file=open("datafiles"+sys.argv[2]+"/"+sys.argv[1]+"justVolcano.txt","w")
for line in open("datafiles"+sys.argv[2]+"/"+sys.argv[1]+"results.txt","r"):
    i=line.split()
    if len(i)==1:
        toWrite="new\n"
    elif float(i[2])<500:
...
myFile=TFile("datafiles"+sys.argv[2]+"/histograms"+sys.argv[1]+"TCADjustVolcano.root","RECREATE")
finalWithCut=TH2D("dummy","dummy",256,0,256,256,0,256)
teller=-1
file.close()
for line in open("datafiles"+sys.argv[2]+"/"+sys.argv[1]+"justVolcano.txt","r"):
    columns=line.split()
    if columns[0]=="new":
        finalWithCut.Write()
        teller+=1
